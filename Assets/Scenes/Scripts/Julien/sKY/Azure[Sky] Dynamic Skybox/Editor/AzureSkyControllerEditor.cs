...
namespace UnityEditor.AzureSky
{
    ...
    public class AzureSkyControllerEditor : Editor
    {
        ...
        void OnEnable()
        {
			...
			//Create Thunder List.
			//-------------------------------------------------------------------------------------------------------
			m_serializedThunderList = serializedObject.FindProperty("thunderList");
			m_reorderableThunderList = new ReorderableList(serializedObject, m_serializedThunderList, false, true, true, true)
			{
				drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
				{
                    m_reorderableThunderList.elementHeight = EditorGUIUtility.singleLineHeight * 5.5f;

                    rect.y += 2;
                    string audioClipName = "None Clip";
					Rect fieldRect = new Rect(rect.x + 75, rect.y + EditorGUIUtility.singleLineHeight, rect.width - 100 - 38, EditorGUIUtility.singleLineHeight);
                    var myList = m_Target.thunderList[index];

                    //Element Text.
                    EditorGUI.LabelField(rect, "Element " + index.ToString(), EditorStyles.boldLabel);

                    //Object Field.
                    GUI.color = m_col2;//Set transparent color.
                    myList.audioClip = (AudioClip)EditorGUI.ObjectField(fieldRect , GUIContent.none, myList.audioClip, typeof(AudioClip), false);
					GUI.color = m_col1;//standard Color.

					//Custom Object Field.
					GUI.color = m_col6;//Red Color.
					if (m_Target.thunderList[index].audioClip)
					{
						GUI.color = m_col5;//Green Color.
						audioClipName = m_Target.thunderList[index].audioClip.name;
					}
					EditorGUI.LabelField(fieldRect, audioClipName, EditorStyles.objectField);
					GUI.color = m_col1;//Return to standard color.
                    fieldRect = new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight, rect.width - 100 - 28, EditorGUIUtility.singleLineHeight);
                    EditorGUI.LabelField(fieldRect, "Audio Clip");

                    //Curve Field.
                    fieldRect = new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight * 2.0f, rect.width - 100 - 28, EditorGUIUtility.singleLineHeight);
                    EditorGUI.LabelField(fieldRect, "Frequency");
                    fieldRect = new Rect(rect.x + 75, rect.y + EditorGUIUtility.singleLineHeight * 2.0f, rect.width - 100 - 55, EditorGUIUtility.singleLineHeight);
                    EditorGUI.CurveField(fieldRect, myList.lightingCurve, m_curveColor, new Rect(0, 0, 1, 1));

                    //Speed Slider.
                    fieldRect = new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight * 3.0f, rect.width - 100 - 28, EditorGUIUtility.singleLineHeight);
                    EditorGUI.LabelField(fieldRect, "Speed");
                    fieldRect = new Rect(rect.x + 75, rect.y + EditorGUIUtility.singleLineHeight * 3.1f, rect.width - 75, EditorGUIUtility.singleLineHeight);
                    myList.lightingSpeed = EditorGUI.Slider(fieldRect, myList.lightingSpeed, 0.0f, 5.0f);

                    //Sound Delay Slider.
                    fieldRect = new Rect(rect.x, rect.y + EditorGUIUtility.singleLineHeight * 4.0f, rect.width - 100 - 28, EditorGUIUtility.singleLineHeight);
                    EditorGUI.LabelField(fieldRect, "Audio Delay");
                    fieldRect = new Rect(rect.x + 75, rect.y + EditorGUIUtility.singleLineHeight * 4.1f, rect.width - 75, EditorGUIUtility.singleLineHeight);
                    myList.audioDelay = EditorGUI.Slider(fieldRect, myList.audioDelay, 0.0f, 3.0f);

                    //Play Button.
                    if (GUI.Button(new Rect(rect.x + rect.width - 50, rect.y + EditorGUIUtility.singleLineHeight, 50, EditorGUIUtility.singleLineHeight * 2.0f), "Play"))
					{
						if (Application.isPlaying)
                        {
                            m_Target.PlayThunderEffect (index);
                        }
                        else
                        {
                            Debug.Log("To perform a thunder effect, the application must be playing.");
                        }
					}
                    m_Target.thunderList[index] = myList;
                },

				onAddCallback = (ReorderableList l) =>
				{
                    //m_reorderableThunderList.elementHeight = EditorGUIUtility.singleLineHeight * 4.5f;
                    var index = l.serializedProperty.arraySize;
                    l.serializedProperty.arraySize++;
                    l.index = index;
                },
				...
				onRemoveCallback = (ReorderableList l) =>
				{
                    //m_reorderableThunderList.elementHeight = EditorGUIUtility.singleLineHeight * 1.3f;
                    ReorderableList.defaultBehaviours.DoRemoveButton(l);
				},

				drawHeaderCallback = (Rect rect) =>
				{
					EditorGUI.LabelField(rect, "Thunder Effects", EditorStyles.boldLabel);
				},
        ...
        public override void OnInspectorGUI()
        {
            ...
            //GUILayout.Space(60);
            GUILayout.Label("Version 4.3.6", EditorStyles.miniLabel);
            ...
            if (m_Target.editorSettings.showReferencesTab)
            {
                ...
                // Thunder Light.
                GUI.color = m_col5;
                if (!m_Target.thunderDirectionalLight)
                {
                    GUI.color = m_col6;
                }
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Thunder Light", GUILayout.Width(m_labelWidth));
                m_Target.thunderDirectionalLight = (Light)EditorGUILayout.ObjectField(m_Target.thunderDirectionalLight, typeof(Light), true);
                EditorGUILayout.EndHorizontal();
            ...
            if (m_Target.editorSettings.showClimateTab)
            {
				...
				if (m_Target.thunderList.Count > 0)
				{
					GUILayout.Space (-17);
					EditorGUILayout.BeginHorizontal();
					if (GUILayout.Button ("Play Random Thunder Effect", EditorStyles.miniButton))
					{
                        if (Application.isPlaying)
                        {
                            m_Target.PlayThunderEffectRandom();
                        }
                        else
                        {
                            Debug.Log("To perform a thunder effect, the application must be playing.");
